...
def build_unet(input_shape):
    ...
    outputs = Conv2D(1, (1, 1), activation='linear')(c5) # 1 channel for Dx
...
def plot_predictions(model, x_test, y_test_scaled, scaler, grid_extent, num_samples=2):
    """ MODIFIED: Plots un-scaled actual vs. predicted results for Dx with BC overlay. """
    if len(x_test) < num_samples:
        num_samples = len(x_test)
        print(f"Warning: Only {num_samples} test samples available to plot.")

    predictions_scaled = model.predict(x_test)
    h, w, c = predictions_scaled.shape[1], predictions_scaled.shape[2], 1
    ...
    for i in range(num_samples):
        ...
        fig.suptitle(f'Sample {i+1}: Actual vs. Predicted Displacement Dx', fontsize=16)
        ...
        vmin, vmax = np.percentile(combined_dx, [2, 98])

        ax = axes[0]
        im = ax.imshow(y_test_actual[i, :, :, 0], cmap='RdYlGn_r', vmin=vmin, vmax=vmax, origin='lower', extent=grid_extent)
        ax.set_title('Actual Displacement Dx')
        fig.colorbar(im, ax=ax)

        ax = axes[1]
        im = ax.imshow(predictions_actual[i, :, :, 0], cmap='RdYlGn_r', vmin=vmin, vmax=vmax, origin='lower', extent=grid_extent)
        ax.imshow(bc_mask, cmap='binary', origin='lower', extent=grid_extent, alpha=0.5) 
        ax.set_title('Predicted Displacement Dx (with BCs)')
        fig.colorbar(im, ax=ax)
...
def plot_error_maps(model, x_test, y_test_scaled, scaler, grid_extent, num_samples=2):
    """ MODIFIED: Calculates and plots the un-scaled error for Dx with BC overlay. """
    if len(x_test) < num_samples:
        num_samples = len(x_test)
    ...
    for i in range(num_samples):
        ...
        fig.suptitle(f'Sample {i+1}: Prediction Error Dx (Actual - Predicted)', fontsize=16)
        ...
        ax.imshow(bc_mask, cmap='binary', origin='lower', extent=grid_extent, alpha=0.5)
        ax.set_title('Dx Error (with BCs)')
        fig.colorbar(im, ax=ax)
